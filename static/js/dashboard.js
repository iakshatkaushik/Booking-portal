document.addEventListener('DOMContentLoaded', () => {
    ...
    window.showSection = function(sectionId) {
        ...
        if (sectionId === 'slotManagementSection') {
            ...
            populateAttendanceSlots(); // Populate for marking
            populateExportAttendanceSlots(); // Populate for export filter
            ...
            document.getElementById('exportAttendanceSubSubgroup').innerHTML = '<option value="">All Sub-subgroups</option>';
...
function generateTimeSlots() {
    ...
    const selectAllSubSubgroupsCheckbox = document.getElementById('selectAllSubSubgroups'); // Added

    // Added event listener for "Select All" checkbox
    selectAllSubSubgroupsCheckbox.addEventListener('change', (event) => {
        const isChecked = event.target.checked;
        subSubgroupCheckboxesDiv.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
    ...
    async function downloadAttendanceExport(url) {
        ...
        throw new Error('Server returned JSON: ' + (data.message || JSON.stringify(data)));
        } else {
    ...
    window.editSlot = async function(id) {
        try {
            ...
            const allCheckboxes = subSubgroupCheckboxesDiv.querySelectorAll('input[type="checkbox"]');
    ...
    const exportAttendanceSlotSelect = document.getElementById('exportAttendanceSlot');
    const exportAttendanceSubSubgroupSelect = document.getElementById('exportAttendanceSubSubgroup');
    ...
    const exportAttendanceStatus = document.getElementById('exportAttendanceStatus');
    const exportAttendanceError = document.getElementById('exportAttendanceError');
    ...
    async function populateExportAttendanceSlots() {
        try {
            ...
            exportAttendanceSlotSelect.innerHTML = '<option value="">All Slots</option>';
    ...
    async function populateExportSubSubgroupSelect(allSlots = []) {
        exportAttendanceSubSubgroupSelect.innerHTML = '<option value="">All Sub-subgroups</option>';
        ...
        if (selectedSlotId) {
                ...
                try {
                    ...
                    groups.forEach(group => {
                        group.subSubgroups.forEach(ssgName => {
                            ...
                            const ssgObj = group.sub_subgroups_full ? group.sub_subgroups_full.find(s => s.name === ssgName) : null;
                            if (ssgObj) {
                    ...
                    const allSsgResponse = await fetch(`${API_BASE_URL}/subsubgroups/all`); // Assuming a new endpoint for this
                    if (allSsgResponse.ok) {
                        ...
                        console.warn("Could not fetch all sub-subgroups to build name-to-ID map.");
                    }


                    selectedSlot.subSubgroups.forEach(ssgName => {
                        const option = document.createElement('option');
                        option.value = ssgNameToIdMap[ssgName] || ssgName; // Use ID if found, else name as fallback (will fail backend)
            ...
            try {
                ...
                if (allSsgResponse.ok) {
                    ...
                    console.warn("Could not fetch all sub-subgroups to build name-to-ID map for 'All Sub-subgroups'.");
                }


                Array.from(uniqueSubSubgroups).sort().forEach(ssgName => {
                    const option = document.createElement('option');
                    option.value = ssgNameToIdMap[ssgName] || ssgName; // Use ID if found
                    option.textContent = ssgName;
                    exportAttendanceSubSubgroupSelect.appendChild(option);
                });

            } catch (error) {
                console.error("Error fetching all sub-subgroups for export filter:", error);
            }
        }
    ...
    */
    ...
    exportAttendanceSlotSelect.addEventListener('change', async () => {
        ...
        try {
            ...
            exportAttendanceError.textContent = `Failed to export attendance: ${error.message}`;
